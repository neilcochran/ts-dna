...
describe('InvalidCodonError', () => {
  ...
  describe('invalid codon lengths', () => {
    test('handles too short codon sequences', () => {
      const error = new InvalidCodonError('Codon must be exactly 3 nucleotides, got 1', 'A');
    ...
    test('handles two nucleotide sequences', () => {
      const error = new InvalidCodonError('Codon must be exactly 3 nucleotides, got 2', 'AU');
    ...
    test('handles too long codon sequences', () => {
      const error = new InvalidCodonError('Codon must be exactly 3 nucleotides, got 4', 'AUGC');
    ...
    test('handles empty codon sequence', () => {
      const error = new InvalidCodonError('Codon cannot be empty', '');
  ...
  describe('invalid nucleotides in codons', () => {
    test('handles invalid nucleotides in RNA codons', () => {
      const error = new InvalidCodonError('Invalid nucleotide T found in RNA codon', 'ATG');
    ...
    test('handles special characters in codons', () => {
      const error = new InvalidCodonError('Invalid character - found in codon', 'AU-');

      expect(error.codonSequence).toBe('AU-');
    });

    test('handles numbers in codons', () => {
      const error = new InvalidCodonError('Invalid character 1 found in codon', 'AU1');

      expect(error.codonSequence).toBe('AU1');
    });

    test('handles mixed case with invalid characters', () => {
      const error = new InvalidCodonError('Invalid character found in codon', 'AuX');
  ...
  describe('valid RNA codons that cause other errors', () => {
    test('handles unknown codon mapping', () => {
      const error = new InvalidCodonError('Codon NNN does not map to any amino acid', 'NNN');
    ...
    test('handles stop codons in wrong context', () => {
      const error = new InvalidCodonError('Stop codon UAA found in middle of sequence', 'UAA');
  ...
  describe('edge cases', () => {
    test('handles unicode characters', () => {
      const error = new InvalidCodonError('Invalid unicode character in codon', 'AU🧬');

      expect(error.codonSequence).toBe('AU🧬');
    });

    test('handles whitespace in codons', () => {
      const error = new InvalidCodonError('Whitespace not allowed in codons', 'AU ');

      expect(error.codonSequence).toBe('AU ');
    });

    test('handles tabs and newlines', () => {
      const error = new InvalidCodonError('Control characters not allowed in codons', 'AU\\t');

      expect(error.codonSequence).toBe('AU\\t');
    });

    test('handles null-like strings', () => {
      const error = new InvalidCodonError('Invalid null-like codon', 'null');
  ...
  describe('error message handling', () => {
    ...
    test('handles complex error messages', () => {
      const complexMessage =
        'Codon "AUX" contains invalid nucleotide "X" at position 2. Valid RNA nucleotides are A, U, C, G.';
  ...
  describe('codon context errors', () => {
    ...
    test('handles premature stop codon errors', () => {
      const error = new InvalidCodonError('Premature stop codon UAG found', 'UAG');
