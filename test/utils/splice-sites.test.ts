...
describe('Splice site utilities', () => {
  ...
  describe('validateSpliceSites', () => {
    ...
    test('rejects intron with invalid donor site', () => {
      ...
      const donorError = result.invalidIntrons.find(err =>
        err.reason.includes("Invalid donor site 'AT'"),
      );
    ...
    test('rejects intron with both invalid sites', () => {
      ...
      const acceptorError = result.invalidIntrons.find(err =>
        err.reason.includes("Invalid acceptor site 'TT'"),
      );
      expect(donorError).toBeDefined();
  ...
  describe('findPotentialSpliceSites', () => {
    ...
    test('finds multiple potential introns', () => {
      ...
      const validIntrons = potentialIntrons.filter(intron => intron.end - intron.start >= 4);
    ...
    test('sorts results by start position', () => {
      ...
      for (let i = 1; i < potentialIntrons.length; i++) {
        expect(potentialIntrons[i].start).toBeGreaterThanOrEqual(potentialIntrons[i - 1].start);
      }
    });
  });
