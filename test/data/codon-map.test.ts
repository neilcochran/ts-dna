...
describe('Codon Map Data', () => {
    ...
    describe('CODON_TO_SLC_MAP', () => {
        ...
        test('amino acids with multiple codons have correct counts', () => {
            ...
            const leucineCodens = Object.entries(CODON_TO_SLC_MAP)
                .filter(([_, slc]) => slc === 'L');
            expect(leucineCodens).toHaveLength(6);

            // Serine has 6 codons
            const serineCodens = Object.entries(CODON_TO_SLC_MAP)
                .filter(([_, slc]) => slc === 'S');
            expect(serineCodens).toHaveLength(6);

            // Arginine has 6 codons
            const arginineCodens = Object.entries(CODON_TO_SLC_MAP)
                .filter(([_, slc]) => slc === 'R');
        ...
        test('all four-fold degenerate amino acids have 4 codons', () => {
            ...
            fourFoldDegenerateAA.forEach(slc => {
                const codons = Object.entries(CODON_TO_SLC_MAP)
        ...
        test('isoleucine has 3 codons (special case)', () => {
            const isoleucineCodens = Object.entries(CODON_TO_SLC_MAP)
                .filter(([_, slc]) => slc === 'I');
            expect(isoleucineCodens).toHaveLength(3);
            expect(isoleucineCodens.map(([codon, _]) => codon).sort())
                .toEqual(['AUA', 'AUC', 'AUU']);
        });
    });
    ...
    describe('data consistency between maps', () => {
        ...
        test('codon counts are consistent between maps', () => {
            Object.entries(SLC_ALT_CODONS_MAP).forEach(([slc, codons]) => {
                const codonToSLCCount = Object.entries(CODON_TO_SLC_MAP)
                    .filter(([_, mappedSLC]) => mappedSLC === slc).length;

                if (codons.length !== codonToSLCCount) {
                    console.log(`Mismatch for ${slc}: ALT_CODONS=${codons.length}, CODON_TO_SLC=${codonToSLCCount}`);
                    console.log(`  ALT_CODONS: ${JSON.stringify(codons)}`);
                    console.log(`  CODON_TO_SLC: ${JSON.stringify(Object.entries(CODON_TO_SLC_MAP).filter(([_, mappedSLC]) => mappedSLC === slc).map(([codon, _]) => codon))}`);
                }
        ...
        test('total codon count is 61 across all maps', () => {
            ...
            const totalAltCodons = Object.values(SLC_ALT_CODONS_MAP)
                .reduce((sum, codons) => sum + codons.length, 0);
