 ...
 * ```
 */
export function validateSpliceSites(sequence: string, introns: GenomicRegion[]): {
    isValid: boolean;
    ...
    for (const intron of introns) {
        ...
        if (intronSequence.length < 4) {
            invalidIntrons.push({
                intron,
                reason: `Intron too short (${intronSequence.length} bases). Minimum length is 4 for GT-AG splice sites.`,
        ...
        if (!SPLICE_ACCEPTOR_SEQUENCES.includes(acceptor as typeof SPLICE_ACCEPTOR_SEQUENCES[number])) {
            invalidIntrons.push({
                intron,
 ...
 * @param maxIntronLength - Maximum intron length (default: 10000)
 * @returns Array of potential intron regions with GT-AG splice sites
 */
export function findPotentialSpliceSites(
    sequence: string,
    minIntronLength: number = 4,
    maxIntronLength: number = 10000
): GenomicRegion[] {
    ...
    for (let i = 0; i <= sequence.length - 2; i++) {
        if (sequence.substring(i, i + 2) === 'GT') {
            gtPositions.push(i);
        }
    }
    ...
    for (let i = 0; i <= sequence.length - 2; i++) {
        if (sequence.substring(i, i + 2) === 'AG') {
            agPositions.push(i);
        }
    }
