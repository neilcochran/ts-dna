...
export class MRNA extends RNA {
    ...
    constructor(
        ...
        if (codingStart < 0 || codingEnd > sequence.length || codingStart >= codingEnd) {
            throw new Error(`Invalid coding sequence boundaries: start=${codingStart}, end=${codingEnd}, sequence length=${sequence.length}`);
        }
        ...
        if (extractedCodingSeq !== codingSequence) {
            throw new Error(`Coding sequence mismatch: expected '${codingSequence}', found '${extractedCodingSeq}' at positions ${codingStart}-${codingEnd}`);
        }
    ...
    isFullyProcessed(): boolean {
        return this.fivePrimeCap &&
               this.polyATail.length >= 10 &&
               this.codingSequence.length >= 3;
    }
