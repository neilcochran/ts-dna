...
export class SpliceVariantPatterns {
  ...
  static exonSkipping(
    ...
    return {
      ...
      includedExons,
      description:
        description ?? `Skips exon${skippedExons.length > 1 ? 's' : ''} ${skippedExons.join(', ')}`,
    };
  }
   ...
   * @param description - Optional description
   */
  static truncation(name: string, exonCount: number, description?: string): SpliceVariant {
   ...
   * @param description - Optional description
   */
  static minimal(name: string, essentialExons: number[], description?: string): SpliceVariant {
    return {
      name,
   ...
   * @param description - Optional description
   */
  static fullLength(name: string, totalExons: number, description?: string): SpliceVariant {
