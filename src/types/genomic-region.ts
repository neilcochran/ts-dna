...
export function validateExons(
    ...
    if (exons.length === 1) {
        ...
        if (!isValidGenomicRegion(exon)) {
            return { success: false, error: `Exon has invalid coordinates: start=${exon.start}, end=${exon.end}` };
        }
        if (exon.end > sequenceLength) {
            return { success: false, error: `Exon extends beyond sequence length: end=${exon.end}, sequence length=${sequenceLength}` };
        }
    ...
    for (let i = 0; i < exons.length; i++) {
        ...
        if (!isValidGenomicRegion(exon)) {
            return { success: false, error: `Exon ${i} has invalid coordinates: start=${exon.start}, end=${exon.end}` };
        }

        if (exon.end > sequenceLength) {
            return { success: false, error: `Exon ${i} extends beyond sequence length: end=${exon.end}, sequence length=${sequenceLength}` };
        }
...
export class IntervalTree {
    ...
    private searchOverlaps(node: IntervalNode | null, query: GenomicRegion, result: GenomicRegion[]): void {
        if (!node) return;
