 ...
 *
 * The module-private `unsafe*` factories and the `UNSAFE_*_KEY` symbols are deliberately
 * excluded from this barrel. Other code under `src/` can import them from their respective
 * `./OkazakiFragment.js` / `./RNAPrimer.js` / `./internal-keys.js` paths when it can prove
 * the inputs are well-formed; package consumers cannot reach them.
 *
 * Replication-specific biological constants (primer length bounds, polymerase speed factors)
 * live inline at `./biological-constants.js` and are not re-exported here; they are
 * implementation details of the simulation rather than part of its public surface.
 */
